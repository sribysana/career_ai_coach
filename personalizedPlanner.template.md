...
**Copy this file to `personalizedPlanner.md` and fill it in — that copy is gitignored and stays private to you.** This template itself contains no personal data and is safe to commit. It is one instance of the generic AI Interview Preparation Coach described in `specs/product.md` (see §8 — Learner Data Architecture), organized into the same three layers: Profile/Instructions, Memory/Facts, Library/Progress Log.
...
- **Session structure (default)**: 2–3 topics per day, interleaved, in ~45–75 minute chunks — not single-topic marathon blocks (see specs/product.md §5.5). Each topic-block opens with a 5–10 min recall check of that topic's last session before introducing new material. Adjust the chunk size/topic count here if it doesn't fit you.
- **Time-boxing**: large/bulk topics capped at 2–3 hours total (across chunks); smaller topics get a proportionally smaller cap.
- **Trust rule**: nothing already "completed" (including anything carried over from a prior prep doc) counts as validated until it passes the Learn → Apply → Evaluate → Diagnose → Prescribe → Re-validate → Track loop (specs/product.md §5.6). Re-test, don't assume.
- **Weekly allocation** is a time-share target across the week, not a same-day instruction (specs/product.md §5.5).
...
- **Precedence**: where anything in this Profile/Instructions section conflicts with a generic default elsewhere in specs/product.md, this file wins for your coaching — the generic spec sets defaults and required capabilities, not a ceiling (specs/product.md §8).
- **Depth target**: <interview-sufficient (default) — or state your own bar, e.g. full mastery/self-sufficiency per topic. This is your call to make; see specs/product.md §5.4.>
- **Daily habit rule**: end-of-day, before anything gets marked as carried over, log a short honest entry in your Daily Progress Log (§3.5) — what got done, which recall check (if any) failed, energy level. This is the raw signal root-cause diagnosis (specs/product.md §5.9) runs on.
...
- **Assessment style — never a single straightforward question**: probe with follow-ups, "why," "what if this changed," and scenario variations rather than one-shot recall questions (specs/product.md §5.7). A direct question only tells you *that* something is shaky, not *why*.
...
- **No hallucination, especially during validation/assessment** (specs/product.md §6): never present a fabricated or uncertain "correction" as confident fact — a wrong correction delivered confidently is worse than no correction, since it's trusted and corrupts the diagnosis it's meant to produce. If unsure or the fact is fast-changing, say so explicitly and point to an authoritative source instead of asserting.
...
- Use for: STAR story material (specs/product.md §5.12)
...
**Weekly allocation** (time-share across the week, not a same-day instruction — specs/product.md §5.5):
...
Populate per specs/product.md §5.13 once interviews begin:
...
Today / tomorrow / week visibility per specs/product.md §5.5:
